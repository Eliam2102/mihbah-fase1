...
async function clean() {
  ...
  await sql.begin(async (trx) => {
    ...
    // 6. Repartos BM Corp (depends on ventas_bmcorp)
    ...
    // 18. Usuarios Portal
    const d19 = await trx`DELETE FROM usuarios_portal`
    console.log(`  Deleted ${d19.count} rows from usuarios_portal`)
