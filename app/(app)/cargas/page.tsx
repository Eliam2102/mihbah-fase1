...
import { Upload, Plus, CheckCircle, AlertCircle, XCircle, FileSpreadsheet } from 'lucide-react'
import { cn } from '@/lib/utils'

export const metadata = { title: 'Cargas Excel · Universo Jade' }

export default async function CargasGlobalPage() {
  const user = await requireUser()
  const tenantId = user.tenantId!

  // List all uploads for the tenant (global view)
  const uploads = await listUploads(tenantId)
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Historial de importaciones del archivo maestro MK1
          </p>
        </div>
        <Link
          href="/cargas/nueva"
      ...
      {uploads.length === 0 ? (
        <div className="border-border bg-card flex flex-col items-center justify-center gap-4 rounded-xl border border-dashed py-20 text-center">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Importa tu archivo Excel maestro (MK1) para comenzar.
            </p>
          </div>
          <Link
            href="/cargas/nueva"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-border bg-muted/50 border-b">
                ...
                <th className="text-destructive px-4 py-3 text-center text-xs font-semibold tracking-wide uppercase">
                  Errores
                </th>
                ...
                <tr key={u.id} className="border-border hover:bg-muted/30 border-b last:border-0">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-foreground font-medium">{u.filename}</span>
                    </div>
                  </td>
