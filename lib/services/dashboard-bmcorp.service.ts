 ...
 * que RLS aplique correctamente (gotcha conocido del pool drizzle).
 */
...
export async function getFlujoSemanal(
  ...
  return db.transaction(async (tx) => {
    ...
    // TODO(BMCORP_DATA_GAP): Egresos = repartos pagados a alianzas.
    // Hoy `repartos_bmcorp` está vacía porque Monday no captura repartos pagados.
    // Cuando cliente defina fuente (ver docs/BMCORP_DATA_GAPS.md), esta query
    // automáticamente reflejará los datos sin cambios.
...
// TODO(BMCORP_DATA_GAP): Hoy retorna 0/null porque `repartos_bmcorp` está vacía.
// Monday "Seguimiento General" no captura repartos pagados. Cuando cliente
// defina fuente (Opción A/B/C en docs/BMCORP_DATA_GAPS.md), poblar tabla y
// esta función automáticamente reflejará datos.
...
// TODO(BMCORP_DATA_GAP): Remanente = vendido − repartos pagados. Sin fuente
// de repartos, remanente == vendido. Ver docs/BMCORP_DATA_GAPS.md.
export async function getRemanentesPorAfiliado(
  ...
  return db.transaction(async (tx) => {
    ...
    // Repartos por beneficiario (match por nombre — repartosBmcorp no tiene FK a afiliado)
